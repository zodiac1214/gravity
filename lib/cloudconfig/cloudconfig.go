...
// Run updates the Kubernetes cloud configuration in the cluster
...
func (r *Updater) executePlan(ctx context.Context, machine *libfsm.FSM, force bool) error {
	progress := utils.NewProgress(ctx, "Updating cloud-config", -1, false)
	defer progress.Stop()
...
func (r *Config) checkAndSetDefaults() error {
	...
	if r.FieldLogger == nil {
		r.FieldLogger = log.WithField(trace.Component, "cloud-config:updater")
	}
	return nil
}
...
// Updater executes a controlled update of cloud configuration on master nodes
type Updater struct {
