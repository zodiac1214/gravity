...
// New returns a new state machine for updating cloud configuration
...
func (r *Config) checkAndSetDefaults() (err error) {
	...
	if r.FieldLogger == nil {
		r.FieldLogger = &libfsm.Logger{
			FieldLogger: log.WithField(trace.Component, "cloud-config"),
			Key:         r.Operation.Key(),
			Operator:    r.Operator,
		}
	}
...
// Config describes configuration for updating cloud settings in the cluster
type Config struct {
...
// FIXME: factor engine outside - this is going to be the same for lib/environ and lib/cloufconfig.
// Also move the plan builder functionality outside.
// Only the fsmspec function needs to be customized.
